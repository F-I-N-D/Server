...
parser.add_argument('--droneFile', '-d', type = str, default = 'drones.csv', help = "the file which specifies the drones")

# Create server instance and start the server
# This file needs te be ran directly and can't be inported
if __name__ == '__main__':
    args = parser.parse_args(sys.argv[1:])
    server = Server(args.droneFile, args.camera, args.noGui)
    server.start()